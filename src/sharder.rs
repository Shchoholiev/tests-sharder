use std::{cmp::max, collections::{BTreeMap, BinaryHeap}};
...
pub fn shard_tests(tests: Vec<Test>, target_shard_time_ms: u32) -> Vec<Vec<Test>> {
    ...
    let longest_test_duration: u32 = tests_durations
      .iter()
      .max()
      .copied()
      .unwrap_or(0);
    ...
    let n_shards: u32 = tests_durations
      .iter()
      .sum::<u32>()
      .div_ceil(shard_time_ms);

    let mut shards: BTreeMap<(u32, usize), Vec<Test>> = 
      (0..n_shards as usize)
        .map(
              |id| 
              (
    ...
    while let Some(test) = tests_sorted.pop() {
        ...
        let Some((remaining_ms, shard_id)) = candidate_key else { continue };

        let mut shard_tests = shards
          .remove(&candidate_key.unwrap())
          .unwrap();
    ...
    let result: Vec<Vec<Test>> = shards
      .into_values()
      .collect();
    return result
} 
